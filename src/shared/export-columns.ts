/**
 * OpenPDR — Shared CSV export column definitions
 *
 * Column types, accessors, and the canonical column list used by both
 * the Electron (streaming) and browser (Blob) CSV export implementations.
 */
...
export const num = (key: keyof TelemetryStore) =>
...
export const sparse = (key: keyof TelemetryStore) =>
...
export const str = (key: keyof TelemetryStore) =>
...
export const CSV_HEADER = COLUMNS.map(c => c.name).join(',') + '\n'
export const COL_COUNT = COLUMNS.length
export const BATCH_SIZE = 2000

export function formatRow(store: TelemetryStore, i: number): string {
